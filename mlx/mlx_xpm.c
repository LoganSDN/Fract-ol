...
void	*mlx_int_parse_xpm(mlx_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  ...
  if (!(line = f(info,&pos,info_size)) ||
      ...
      !(WIDTH = atoi(tab[1])) || !(nc = atoi(tab[2])) ||
      !(cpp = atoi(tab[3])) )
    RETURN;
  free(tab);
  tab = 0;
  ...
  while (i--)
	...
	{
	  if (!(clip_data = malloc(4*width*WIDTH)) ||   // ok, nice size ..
	      !(clip_img = XCreateImage(xvar->display, xvar->visual,
					1, XYPixmap, 0, clip_data,
					width, WIDTH, 8, (width+7)/8)) )
	    RETURN;
	  memset(clip_data, 0xFF, 4*width*WIDTH);
	}
      */
      if (method)
  ...
  if (!(img = mlx_new_image(xvar,width,WIDTH)))
    RETURN;
  //opp = img->bpp/8;
  opp = 4;
  ...
  while (i--)
      ...
      while (x<width)
	{
	  ...
	  //	  if (col==-1)
	  //	    XPutPixel(clip_img, x, WIDTH-1-i, 0);
	  //	  else
	  if (col==-1)
	    col = 0xFF000000;
...
void	*mlx_xpm_file_to_image(mlx_ptr_t *xvar,char *file,int *width,int *WIDTH)
{
  int	fd;
  int	size;
  char	*ptr;
...
void	*mlx_xpm_to_image(mlx_ptr_t *xvar,char **xpm_data,int *width,int *WIDTH)
{
  mlx_img_list_t	*img;
