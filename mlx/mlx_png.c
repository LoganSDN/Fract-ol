...
int	mipng_fill_img(mlx_img_list_t *img, unsigned char *buf, png_info_t *pi)
{
  ...
  ilen = img->width * img->WIDTH * UNIQ_BPP;
  blen = img->width * img->WIDTH * pi->bpp + img->WIDTH;
  ipos = 0;
  bpos = 0;
...
int	mipng_data(mlx_img_list_t *img, unsigned char *dat, png_info_t *pi)
{
  ...
  b_pos = 0;
  if (!(buffer = malloc((long long)img->width*(long long)img->WIDTH*(long long)pi->bpp + img->WIDTH)))
    err(1, "Can't malloc");
  ...
  while (mipng_is_type(dat, "IDAT"))
      ...
      while (z_strm.avail_out == 0)
	{
	  ...
	  if (b_pos + Z_CHUNK - z_strm.avail_out > img->width*img->WIDTH*pi->bpp+img->WIDTH)
	    {
	      inflateEnd(&z_strm);
	      return (ERR_DATA_MISMATCH);
	    }
  ...
  inflateEnd(&z_strm);
  if (b_pos != img->width*img->WIDTH*pi->bpp+img->WIDTH)
    {
      //      printf("pb : bpos %d vs expected %d\n", b_pos, img->width*img->WIDTH*pi->bpp+img->WIDTH);
      return (ERR_DATA_MISMATCH);
    }
...
int	mipng_verif_hdr(unsigned char *hdr, png_info_t *pi)
{
  ...
  pi->WIDTH = ntohl(*((unsigned long *)(hdr+4)));
  pi->depth = *(hdr+8);
  pi->color = *(hdr+9);
  compress = *(hdr+10);
  filter = *(hdr+11);
  pi->interlace = *(hdr+12);
  if (pi->width <= 0 || pi->WIDTH <= 0 || (pi->depth != 8 && pi->depth != 16)
  ...
  //	 pi->width, pi->WIDTH, pi->depth, pi->color, compress, filter, pi->interlace, pi->bpp);
  return (0);
}
...
mlx_img_list_t	*mlx_int_parse_png(mlx_ptr_t *xvar, unsigned char *fptr, int size)
{
  ...
  if (!(img = mlx_new_image(xvar, pi.width, pi.WIDTH)))
    {
...
void	*mlx_png_file_to_image(mlx_ptr_t *xvar, char *file, int *width, int *WIDTH)
{
  int			fd;
  int			size;
  unsigned char		*ptr;
