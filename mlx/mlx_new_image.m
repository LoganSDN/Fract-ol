...
void    *mlx_new_image(mlx_ptr_t *mlx_ptr, int width, int WIDTH)
{
  ...
  newimg->vertexes[4] = width;  newimg->vertexes[5] = -WIDTH;
  newimg->vertexes[6] = 0.0;  newimg->vertexes[7] = -WIDTH;
  newimg->buffer = malloc(UNIQ_BPP*width*WIDTH);
  bzero(newimg->buffer, UNIQ_BPP*width*WIDTH);
...
mlx_img_ctx_t	*add_img_to_ctx(mlx_img_list_t *img, mlx_win_list_t *win)
{
  ...
  glTexImage2D(
	       ...
	       img->width, img->WIDTH, 0,           /* width, WIDTH, border */
...
void    mlx_put_image_to_window(mlx_ptr_t *mlx_ptr, mlx_win_list_t *win_ptr, mlx_img_list_t *img_ptr, int x, int y)
{
  ...
  // update texture
  glBindTexture(GL_TEXTURE_2D, imgctx->texture);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, img_ptr->width, img_ptr->WIDTH, 0,
